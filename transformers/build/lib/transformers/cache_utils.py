...
##### Code for QuantizedCacheLM and QuantizedCacheVLM -----> VidKV
from .vidkv_quant import triton_quantize_and_pack_along_last_dim, unpack_and_dequant_vcache
...
class QuantizedCacheLM(QuantizedTensorFunction):

    def __init__(self, cache_config: CacheConfig) -> None:
        super().__init__(cache_config)                
        print(self.nbits,"->---------------- QuantizedCacheLM")
    ...
    def update(
        ...
        if len(self.key_cache) < layer_idx:
            ...
            if self.axis_key == -1:
                ...
                if remainder > 0:
                    self._quantized_key_cache.append(self._quantize_key(key_states[:,:,:-remainder,:], axis=self.axis_key))
                    self.key_cache.append(torch.zeros(0, dtype=key_states.dtype, device=key_states.device))
                    self.key_cache[layer_idx] = torch.cat([self.key_cache[layer_idx], key_states[:,:,-remainder:,:]], dim=-2)
                else:
                    self._quantized_key_cache.append(self._quantize_key(key_states, axis=self.axis_key))
                    self.key_cache.append(torch.zeros(0, dtype=key_states.dtype, device=key_states.device))
            else:
                self._quantized_key_cache.append(self._quantize_key(key_states, axis=self.axis_key))
                self.key_cache.append(torch.zeros(0, dtype=key_states.dtype, device=key_states.device))

            if self.axis_value == -1:
                B, H, N, D = value_states.shape
                remainder = N % self.q_group_size
                if remainder > 0:
                    self._quantized_value_cache.append(self._quantize_value(value_states[:,:,:-remainder,:], axis=self.axis_value))
                    self.value_cache.append(torch.zeros(0, dtype=value_states.dtype, device=value_states.device))
                    self.value_cache[layer_idx] = torch.cat([self.value_cache[layer_idx], value_states[:,:,-remainder:,:]], dim=-2)
                else:
                    self._quantized_value_cache.append(self._quantize_value(value_states, axis=self.axis_value))
                    self.value_cache.append(torch.zeros(0, dtype=value_states.dtype, device=value_states.device))
            else:
                self._quantized_value_cache.append(self._quantize_value(value_states, axis=self.axis_value))
                self.value_cache.append(torch.zeros(0, dtype=value_states.dtype, device=value_states.device))
            ...
            dequant_key = self._dequantize_key(self._quantized_key_cache[layer_idx])
            dequant_value = self._dequantize_value(self._quantized_value_cache[layer_idx])
            keys_to_return = [dequant_key, self.key_cache[layer_idx], key_states]
            values_to_return = [dequant_value, self.value_cache[layer_idx], value_states]
            ...
            if (
                ...
                self._quantized_key_cache[layer_idx] = self._quantize_key(keys_to_return, axis=self.axis_key)
                self._quantized_value_cache[layer_idx] = self._quantize_value(
    ...
    def _quantize_key(self, tensor, axis):
        return self._quantize(tensor, axis=axis, group_size=self.q_group_size, bits=self.nbits)

    def _quantize_value(self, tensor, axis):
        return self._quantize(tensor, axis=axis, group_size=self.q_group_size, bits=self.nbits)

    def _dequantize_key(self, q_tensor):
        return self._dequantize(q_tensor, group_size=self.q_group_size, bits=self.nbits, axis=self.axis_key, type=self.compute_dtype)

    def _dequantize_value(self, q_tensor):
        return self._dequantize(q_tensor, group_size=self.q_group_size, bits=self.nbits, axis=self.axis_value, type=self.compute_dtype)


class QuantizedCacheVLM(QuantizedCacheLM):
    """
    Vision: 2025.02.03
    ...
    def __init__(self, cache_config: CacheConfig) -> None:
        ...
        if isinstance(self.nbits, list):
            self.nbits_key = self.nbits[0]
            self.nbits_value = self.nbits[1]
            self.dynamic_key_quantize = True
            self.difficult_ratio = self.nbits_key - 1
        else:
            self.nbits_key = self.nbits
            self.nbits_value = self.nbits
            self.dynamic_key_quantize = False
    ...
    def update(
        ...
        if len(self.key_cache) < layer_idx:
            ...
            if (
                ...
                self._quantized_key_cache[layer_idx] = self._quantize_key(keys_to_return, axis=self.axis_key)
                self._quantized_value_cache[layer_idx] = self._quantize_value(values_to_return, axis=self.axis_value)
                self.key_cache[layer_idx] = torch.zeros(0, dtype=key_states.dtype, device=key_states.device)
                self.value_cache[layer_idx] = torch.zeros(0, dtype=key_states.dtype, device=key_states.device)
            else:
                self.key_cache[layer_idx] = torch.cat([self.key_cache[layer_idx], key_states], dim=-2)
    ...
    def _quantize_key(self, tensor, axis):
        if self.dynamic_key_quantize:
            return self.dynamic_quantize_key(tensor)
        else:
            if self.nbits_key == 1:
                return self._quantize_fft(tensor, axis=self.axis_key, group_size=self.q_group_size, bits=1)
            else:
                return self._quantize(tensor, axis=axis, group_size=self.q_group_size, bits=self.nbits_key)
    
    def _dequantize_key(self, q_tensor):
        if self.dynamic_key_quantize:
            return self.dynamic_dequantize_key(q_tensor)
        else:
            if self.nbits_key == 1:
                return self._dequantize_fft(q_tensor, axis=self.axis_key, group_size=self.q_group_size, bits=1)
            else:
                return self._dequantize(q_tensor, group_size=self.q_group_size, bits=self.nbits_key, axis=self.axis_key)

    def dynamic_quantize_key(self, tensor):
        try:
            assert self.axis_key == -1
            key_axis = tensor.ndim - 1 
        except:
            raise ValueError("dynamic_quantize_key only support axis_key=-1")
        
        D = tensor.shape[-1]
        flattened = tensor.reshape(-1, D)  # shape: [1*B*N, D]
        channel_max = flattened.max(dim=0)[0]
        channel_min = flattened.min(dim=0)[0]
        channel_range = channel_max - channel_min  # shape: [D]

        num_channels = channel_range.numel()
        assert channel_range.shape == (D,)
        k = int(num_channels * self.difficult_ratio)
        if self.difficult_ratio > 0 and k == 0:
            k = 32
        _, topk_indices = torch.topk(channel_range, k, largest=True)
        difficult_mask = torch.zeros_like(channel_range, dtype=torch.bool)
        difficult_mask[topk_indices] = True
        easy_mask = ~difficult_mask

        easy_idx = torch.nonzero(easy_mask, as_tuple=True)[0]
        difficult_idx = torch.nonzero(difficult_mask, as_tuple=True)[0]
        
        q_easy = None
        if easy_idx.numel() > 0:
            tensor_easy = torch.index_select(tensor, dim=self.axis_key, index=easy_idx)
            q_easy = self._quantize_fft(tensor_easy, axis=self.axis_key, group_size=self.q_group_size, bits=self.low_nbits)
        q_difficult = None
        if difficult_idx.numel() > 0:
            tensor_difficult = torch.index_select(tensor, dim=self.axis_key, index=difficult_idx)
            q_difficult = self._quantize(tensor_difficult, axis=self.axis_key, group_size=self.q_group_size, bits=self.high_nbits)
    ...
    def dynamic_dequantize_key(self, q_repr):
        original_shape = q_repr['original_shape']
        easy_idx = q_repr['easy_idx']
        difficult_idx = q_repr['difficult_idx']

        if q_repr['q_easy'] is not None:
            device = q_repr['q_easy'][0].device
        elif q_repr['q_difficult'] is not None:
            device = q_repr['q_difficult'][0].device
        else:
            print("Warning: dynamic_dequantize_key used but no quantized data found!")
            device = torch.device('cpu')

        deq = torch.empty(original_shape, device=device, dtype=torch.float16)

        if q_repr['q_easy'] is not None:
            deq_easy = self._dequantize_fft(q_repr['q_easy'], axis=self.axis_key, group_size=self.q_group_size, bits=self.low_nbits)
            deq = deq.index_copy(self.axis_key, easy_idx, deq_easy)

        if q_repr['q_difficult'] is not None:
            deq_difficult = self._dequantize(q_repr['q_difficult'], group_size=self.q_group_size, bits=self.high_nbits, axis=self.axis_key)
            deq = deq.index_copy(self.axis_key, difficult_idx, deq_difficult)

        assert deq.shape == original_shape
        return deq

    def _quantize_fft(self, tensor, axis=-1, group_size=None, bits=None):
        self.shape_fft = tensor.shape
        B, nh, T, hd = tensor.shape
        v_fft = torch.fft.fft(tensor, dim=-1)
        v_fft_real = torch.view_as_real(v_fft)
        B, nh, T, hd, two = v_fft_real.shape
        v_fft_merged = v_fft_real.reshape(B, nh, T, hd * two)
        return self._quantize(v_fft_merged, group_size=group_size, bits=bits, axis=axis)

    def _dequantize_fft(self, qtensor, axis=-1, group_size=None, bits=None):
        B, nh, T, hd = self.shape_fft
        code, scale, mn = qtensor
        dequant_v_fft = unpack_and_dequant_vcache(code, scale.unsqueeze(-1), mn.unsqueeze(-1), group_size, bits)
        T = dequant_v_fft.shape[-1]
        if axis == -1:
            dequant_v_fft = dequant_v_fft.transpose(2, 3)
        dequant_v_fft = dequant_v_fft.view(B, nh, T, hd, 2)
        v_fft_dequant = torch.view_as_complex(dequant_v_fft.contiguous())
        v_rec = torch.fft.ifft(v_fft_dequant, dim=-1).real
        return v_rec


    def _quantize_value(self, tensor, axis=-1, group_size=None, bits=None):
        
        return self._quantize(tensor, group_size=self.q_group_size, bits=self.nbits_value, axis=axis)
    
    def _dequantize_value(self, qtensor, axis=-1, group_size=None, bits=None):

        return self._dequantize(qtensor, group_size=self.q_group_size, bits=self.nbits_value, axis=axis)





class QuantizedCacheVLMSTP(QuantizedCacheLM):
    ...
    def __init__(self, cache_config: CacheConfig) -> None:
        ...
        self.high_nbits = 2
        try:
            self.low_tokens = cache_config.low_tokens
            self.rel_tokens = cache_config.rel_tokens
        except:
            raise ValueError("QuantizedCacheVLMSTP requires low_tokens and rel_tokens in cache_config. This model is not supported.")
    ...
    def _dequantize_key(self, q_tensor):
        if self.dynamic_key_quantize:
            ...
            if self.nbits == 1:
                ...
                return self._dequantize(q_tensor, group_size=self.q_group_size, bits=self.nbits, axis=self.axis_key)
    ...
    def _quantize_value(self, tensor, axis=-1, group_size=None, bits=None):
        if self.value_mix:
            _, _, N, _ = tensor.shape
            self.value_shape = tensor.shape
            low_index = self.low_tokens[self.low_tokens < N].to(tensor.device)
            index = torch.arange(0, N, device=tensor.device)
            self.rel_tokens = index[~torch.isin(index, low_index)]
            assert self.rel_tokens.shape[0] + low_index.shape[0] == N
            qtensor_low = self._quantize(tensor[:,:,low_index,:], axis=axis, group_size=self.q_group_size, bits=self.high_nbits)
            qtensor_high = self._quantize(tensor[:,:,self.rel_tokens,:], axis=axis, group_size=self.q_group_size, bits=self.nbits_value)
            return [qtensor_low, qtensor_high]
        else:
            return self._quantize(tensor, group_size=self.q_group_size, bits=self.nbits_value, axis=axis)
    
    def _dequantize_value(self, qtensor, axis=-1, group_size=None, bits=None):
        if self.value_mix:
            assert len(qtensor) == 2
            rel_tokens = self._dequantize(qtensor[1], group_size=self.q_group_size, bits=self.nbits_value, axis=axis)
            if rel_tokens.shape[2] != self.rel_tokens.shape[0]:
                self.rel_tokens = self.rel_tokens[:-self.residual_length]
                self.value_shape = self.value_shape[:2] + (self.value_shape[2] - self.residual_length,) + self.value_shape[3:]
            mix_value = torch.zeros(self.value_shape, device=qtensor[0][0].device, dtype=self.compute_dtype)
            mix_value[:,:,self.low_tokens,:] = self._dequantize(qtensor[0], group_size=self.q_group_size, bits=self.high_nbits, axis=axis)
            mix_value[:,:,self.rel_tokens,:] = rel_tokens
            return mix_value
        else:
            return self._dequantize(qtensor, group_size=self.q_group_size, bits=self.nbits_value, axis=axis)
